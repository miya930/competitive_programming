        ...
        do {
                ...
                switch(op_code(ir)) {
                        case MOV : reg[op_regA(ir)] = reg[op_regB(ir)];
                                   break;
                        case ADD : reg[op_regA(ir)] += reg[op_regB(ir)];
                                   break;
                        case SUB : reg[op_regA(ir)] -= reg[op_regB(ir)];
                                   break;
                        case AND : reg[op_regA(ir)] &= reg[op_regB(ir)];
                                   break;
                        case OR  : reg[op_regA(ir)] |= reg[op_regB(ir)];
                                   break;
                        case SL  : reg[op_regA(ir)] <<= 1;
                                   break;
                        case SR  : reg[op_regA(ir)] >>= 1;
                                   break;
                        ...
                        case LDL : reg[op_regA(ir)] = (reg[op_regA(ir)] & 0xff00) | (op_data(ir) & 0x00ff);
                                   break;
                        case LDH : reg[op_regA(ir)] = (reg[op_regA(ir)] & 0x00ff) | (op_data(ir) & 0xff00);
                                   break;
                        case CMP : if (reg[op_regA(ir)] == reg[op_regB(ir)]) 
                                        ...
                                        flag_eq = 0;
                                   break;
                        case JE  : if (flag_eq) pc = op_addr(ir);
                                   break;
