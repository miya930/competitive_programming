...
int calc_num(int cur_x, int cur_y, int d) {
    int ret = 0;

    for (int i = 0; i < MAXCNT; ++i) {
        int dist_s = dist(cur_x, cur_y, rest_x[i], rest_y[i]);
        int dist_g = dist(cur_x, cur_y, dest_x[i], dest_y[i]);
        if (dist_s <= d && dist_g <= d) {
            ret++;
        }
    }
...
int opt_dist(void) {
    ...
    while (r - l > 1) {
        int mid = (r+l)/2;
        int num = calc_num(cur_x, cur_y, mid);
        if (num < PICKUP) l = mid;
        else r = mid;
    }
