...
void Input(istream& is) {
    ...
    for (int i = 0; i < N; i++) {
        degreeInit.push_back(make_pair(degree[0][i], i));
    }
    sort(degreeInit.begin(), degreeInit.end());
...
void Input1(void) {
#if DEBUG
/*    const string TEST_IN_DIR = "test_in/";

    ifstream fin("test_in/0000.txt");
    fin >> N >> M >> D >> K;
    cout << N << M << D << K << endl;*/
#endif
...
void AnsInitializeMod(void) {
    ...
    for (int d = 1; d <= D; d++) {
        ...
        while(1) {
            ...
            /* 残りの次数を計算 */
            int tmp1 = degree[d][v1] - 1;
            int tmp2 = degree[d][v2] - 1;
            if (tmp1 < 0 || tmp2 < 0) continue;
            if ((tmp1 == 0 || tmp2 == 0) && cnt < thres) continue;
...
#endif
        Yakinamashi();
//        cout << "loopcnt = " << loopCnt << "   time = " << endtime - starttime << endl;
    }
#if DEBUG    
    cout << endl;
#endif
